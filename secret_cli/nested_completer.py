...
# ! Enabled meta_dict for Nested completer as well
class NestedCompleter(Completer):
    ...
    @classmethod
    def from_nested_dict(cls, data: NestedDict, meta_dict={}) -> "NestedCompleter":
        """
        ...
        for key, value in data.items():
            if isinstance(value, Completer):
                ...
                options[key] = cls.from_nested_dict(value, meta_dict)
            elif isinstance(value, set):
                options[key] = cls.from_nested_dict({item: None for item in value}, meta_dict)
            else:
